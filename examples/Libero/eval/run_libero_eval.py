...
class GR00TPolicy:
    ...
    def _process_observation(self, obs, lang: str):
        ...
        # 使用欧拉角（与训练数据一致），不是轴角
        rpy = quat2euler(obs["robot0_eef_quat"])
    ...
    def _convert_to_libero_action(
        ...
        # 模型输出 gripper 已经是 [-1, 1] 范围，直接二值化
        # 注意：原生数据训练的模型不需要 normalize_gripper_action
        action_array[-1] = 1.0 if action_array[-1] > 0 else -1.0
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        env, task_description = get_libero_env(task, resolution=128)
