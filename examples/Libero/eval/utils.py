...
def get_libero_env(task, resolution=128):
...
def get_libero_image(obs):
    ...
    # img = img[::-1, ::-1]  # 移除：原生数据训练不需要翻转
    wrist_img = obs["robot0_eye_in_hand_image"]
    # wrist_img = wrist_img[::-1, ::-1]  # 移除：原生数据训练不需要翻转
...
def quat2euler(quat):
    """
    Converts quaternion (w,x,y,z) from robosuite to Euler angles (roll, pitch, yaw) in radians.
    Note: robosuite provides quaternions in (w,x,y,z) format.
    
    Args:
        quat (np.array): (w,x,y,z) quaternion from robosuite
    
    Returns:
        np.array: (roll, pitch, yaw) Euler angles in radians
    """
    from scipy.spatial.transform import Rotation
    # robosuite uses wxyz order, scipy uses xyzw. Convert wxyz to xyzw.
